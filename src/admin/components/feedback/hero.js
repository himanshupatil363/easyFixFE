...
const Hero  = () =>{
    ...
    useEffect(()=>{
        const fetchPosts = async ()=>{
            setLoading(true);
            const res = await axios.get('http://localhost:9990/feedback/all');
            setPosts(res.data);
            setRepo(res.data);
            setLoading(false);
        }
    ...
    return(
        <div className="Hero w-full ml-60 flex flex-col">
            ...
            <Report data={repo} opening={(<p className="text-center text-xl mb-3">Feedback reports</p>)} closing={(<p className="text-center text-xl mt-3">End</p>)}/>
        </div>
    );
}
export default Hero;